...
describe("RecommendationRequestIndexPage tests", () => {
  ...
  test("renders three recommendation request correctly for regular user", async () => {
    setupUserOnly();
    axiosMock
    ...
    const requestorEmail = screen.getByText("sauldiaz@ucsb.edu");
    expect(requestorEmail).toBeInTheDocument();

    const professorEmail = screen.getByText("fgibou@ucsb.edu");
    expect(professorEmail).toBeInTheDocument();

    const explanation = screen.getByText("Recommendation 3");
